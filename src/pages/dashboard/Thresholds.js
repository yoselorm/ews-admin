...
const Thresholds = () => {
    ...
    const handleCreateThreshold = async (e) => {
        ...
        try {
            ...
            toast.error(err || "Failed to create threshold. Please check your inputs.");
        }
    };
    ...
    const handleUpdateThreshold = async (e) => {
        if (e) e.preventDefault();
        try {
            await dispatch(updateThreshold({ id: selectedItem.id, data: formData })).unwrap();

            toast.success("Threshold updated successfully!");

        } catch (err) {
            toast.error(err || "Failed to update threshold. Please check your inputs.");
        }
    };
