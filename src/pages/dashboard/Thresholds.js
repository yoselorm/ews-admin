...
const RowActionsMenu = ({ item, onEdit, onDelete, onViewMatrix }) => {
  ...
  const toggleOpen = () => {
    if (!open && btnRef.current) {
      ...
      setCoords({
        top: rect.bottom + 6,
        left: rect.right - menuWidth, // right-align to the button, like before
      });
    }
    setOpen((o) => !o);
  };
  ...
  useEffect(() => {
    ...
    // Close on scroll/resize so a stale-positioned menu never lingers
      ...
      {open &&
        createPortal(
          <div
            ...
            className="w-48 bg-white border border-slate-100 rounded-xl shadow-lg z-[9999] overflow-hidden py-1"
          >
            <button
              onClick={() => {
                setOpen(false);
              ...
              className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-all"
            >
            ...
            <button
              ...
              className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 transition-all"
            >
              <Trash2 size={15} />
              Delete
            </button>
        ...
        <div className="flex gap-2 items-center">
          <input
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                gestational_age_min_weeks: Number(e.target.value),
              })
            }
          />
          ...
          <input
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                gestational_age_max_weeks: Number(e.target.value),
              })
            }
          />
        </div>
...
const DeleteModal = ({ item, onConfirm, onCancel, actionLoading }) =>
  createPortal(
    <div
      className="fixed inset-0 z-[9999] flex items-center justify-center p-4"
      style={{
...
const Thresholds = () => {
  ...
  const { list, loading, actionLoading, success, error, meta } = useSelector(
  ...
  const handleViewSendMatrix = (item) => {
    navigate(`send-matrix/${item.id}`);
  };
  ...
  const openEdit = (item) => {
    ...
    setFormData({
      ...
      unit: item.unit,
      risk_level: ["low", "moderate", "high"].includes(item.risk_level)
        ? item.risk_level
        : "",
  ...
  const handleCreateThreshold = async (e) => {
    ...
    try {
      await dispatch(createThreshold(formData)).unwrap();

      toast.success("Threshold created successfully!");
    } catch (err) {
      toast.error(
        err || "Failed to create threshold. Please check your inputs.",
      );
    }
  };
  ...
  const handleUpdateThreshold = async (e) => {
    ...
    try {
      await dispatch(
        updateThreshold({ id: selectedItem.id, data: formData }),
      ).unwrap();

      toast.success("Threshold updated successfully!");
    } catch (err) {
      toast.error(
        err || "Failed to update threshold. Please check your inputs.",
      );
    }
  };
  ...
  const handleDeleteThreshold = async () => {
    try {
      await dispatch(deleteThreshold(selectedItem.id)).unwrap();

      toast.success("Threshold deleted successfully!");

      closeModal();
    } catch (err) {
      toast.error(err || "Failed to delete threshold. Please try again.");
    }
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Define environmental limits that trigger health alerts.
          </p>
        </div>
        <button
          onClick={() => {
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <code className="bg-slate-100 px-2 py-1 rounded text-purple-600 text-xs font-mono">
                      {item.parameter} {item.operator} {item.threshold_value}
                      {item.unit}
                    </code>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right ">
                    <RowActionsMenu
                      ...
                      onViewMatrix={handleViewSendMatrix}
                    />
                  </td>
                </tr>
              ))
