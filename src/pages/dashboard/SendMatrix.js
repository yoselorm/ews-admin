...
const getSendStatus = (dispatch) => {
  ...
  if (dispatch.status === "dispatched") {
    return {
      key: "dispatched",
      text: `Dispatched at ${formatTimePart(dispatch.dispatched_at)}`,
    };
  }
...
const StatusPill = ({ dispatch }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${meta.badge}`}
    >
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 flex flex-col">
      <div className="flex flex-col items-center gap-2 mb-2.5">
        <span
          className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide border ${meta.badge}`}
        >
          <span className={`w-1.5 h-1.5 rounded-full ${meta.dot}`} />
          {meta.label}
        </span>
    ...
    {view === 'sms' ? (
        <Smartphone size={12} className="text-slate-400 float-left mr-1.5 mt-0.5" />
    ) : (
        <Bell size={12} className="text-slate-400 float-left mr-1.5 mt-0.5" />
    )}
...
const SendRow = ({ tierLabel, send }) => {
  ...
  const orderedMessages = ROLE_ORDER.map((role) => ({
    role,
    message: messages.find((m) => m.role === role),
      ...
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold text-slate-700">
            {formatDatePart(send.scheduled_for)} ·{" "}
            {formatTimePart(send.scheduled_for)}
          </span>
      ...
      {orderedMessages.length > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-200 p-5 text-center">
          <p className="text-xs text-slate-400">
            No precaution content configured for this send yet.
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div className="rounded-3xl border border-slate-100 bg-white p-5 mb-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <span
          className={`px-3 py-1 rounded-full text-xs font-black tracking-widest border ${meta.badge}`}
        >
          {meta.label}
        </span>
...
const SendMatrix = () => {
  ...
  const totalMessages = data
    ? TIER_ORDER.reduce((sum, tier) => {
        ...
        return (
          sum + sends.reduce((s, send) => s + (send.messages?.length || 0), 0)
        );
      }, 0)
    : 0;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm">
          View the full scheduled send plan for a threshold. Adjust community or
          date and apply to refine.
        </p>
      </div>
