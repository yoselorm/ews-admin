...
const LanguageManagement = () => {
    ...
    const handleUpsert = async (e) => {
        ...
        try {
            ...
            toast.success(`Language ${targetLang ? 'updated' : 'created'} successfully`);
            setIsUpsertModalOpen(false);
        } catch (err) {
            toast.error(err || `Failed to ${targetLang ? 'update' : 'create'} language`);
        }
    };
...
const confirmDelete = async () => {
    ...
    try {
        await dispatch(deleteLanguage(targetLang.id)).unwrap();

        toast.success("Language deleted successfully");
        
        setIsDeleteModalOpen(false);
    } catch (err) {
        toast.error(err || "Failed to delete language");
    }
};
        ...
        <div className=" bg-[#F9FAFB] min-h-screen">
            ...
            <div className="flex justify-between items-center mb-12">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">System Languages</h1>
